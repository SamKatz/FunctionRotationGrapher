...
public class TwoFunc2D{
	
	private int maxfps = 60;
	private Camera2D cam;
	private float ratio = 1.0f;
	private ExpressionBuilder EBOne, EBTwo;
	private double dx, lowerx, upperx;
    
	public TwoFunc2D(ExpressionBuilder EBOne, ExpressionBuilder EBTwo, double precision, double camspeed, double startx, double endx){
    	this.EBOne = EBOne;
    	this.EBTwo = EBTwo;
    	this.dx = precision;
    	this.lowerx = startx;
    ...
    private double funcOne(double x){
    	try {
			return (float) EBOne.withVariable("x", x).withCustomFunction(Main.logten).build().calculate();
		}
    	catch(Exception e) {
			e.printStackTrace();
		}
    ...
    private double funcTwo(double x){
    	try {
			return (float) EBTwo.withVariable("x", x).withCustomFunction(Main.logten).build().calculate();
		}
    	catch(Exception e) {
			e.printStackTrace();
		}
